...
func main() {
	...
	if dbURL == "" {
		logger.Fatal(context.Background(), "USERS_DB_URL is not set")
	}
	...
	if len(os.Args) > 1 && os.Args[1] == "migrate" {
		...
		logger.Println(context.Background(), "Starting migration process for users...")

		if err := database.RunMigrations(db, migrations.FS, "users"); err != nil {
			logger.Fatalf(context.Background(), "Fatal error running users migrations: %v", err)
		}
	...
	logger.Printf(context.Background(), "Starting server on port %s...", port)
