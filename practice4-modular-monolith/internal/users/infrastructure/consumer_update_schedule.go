...
func (w *ConsumerWorker) processUpdateSchedule(ctx context.Context, d amqp.Delivery) {
	...
	if err := json.Unmarshal(d.Body, &cmd); err != nil {
		logger.Printf(ctx, "[Users Consumer] Invalid JSON for UpdateSchedule: %v", err)
		d.Nack(false, false)
		return
	}
	...
	if err != nil {
		logger.Printf(ctx, "[Users Consumer] Failed to begin transaction for Workflow %s: %v", cmd.WorkflowID, err)
		d.Nack(false, true)
		return
	}
	defer tx.Rollback()
	...
	if !w.checkInbox(ctx, tx, inboxEventID) {
		logger.Printf(ctx, "[Users Consumer] Command %s already processed. Skipping.", inboxEventID)
		d.Ack(false)
		return
	}
	...
	if err != nil {
		logger.Printf(ctx, "[Users Consumer] Failed to update user meetups in DB: %v", err)
		d.Nack(false, true)
		return
	}
	...
	if err != nil {
		logger.Printf(ctx, "[Users Consumer] Failed to marshal reply event: %v", err)
		d.Nack(false, true)
		return
	}
	...
	if err != nil {
		logger.Printf(ctx, "[Users Consumer] Failed to save outbox event: %v", err)
		d.Nack(false, true)
		return
	}

	if err := tx.Commit(); err != nil {
		logger.Printf(ctx, "[Users Consumer] Failed to commit transaction: %v", err)
		d.Nack(false, true)
		return
	}

	logger.Printf(ctx, "[Users Consumer] Successfully updated schedule for User %s (Workflow %s)", cmd.UserID, cmd.WorkflowID)
	d.Ack(false)
}
...
func (w *ConsumerWorker) handleUpdateErrorFlow(ctx context.Context, err error, tx *sql.Tx, d amqp.Delivery, cmd commands.UpdateSchedule) {
	...
	if errors.As(err, &sharedErr) && sharedErr.Type == shared.ErrorTypeNotFound {
		...
		logger.Printf(ctx, "[Users Consumer] User not found. Failing workflow %s.", cmd.WorkflowID)
		...
		if saveErr != nil {
			logger.Printf(ctx, "[Users Consumer] Failed to save failure outbox event: %v", saveErr)
			d.Nack(false, true)
			return
		}

		if commitErr := tx.Commit(); commitErr != nil {
			logger.Printf(ctx, "[Users Consumer] Failed to commit failure transaction: %v", commitErr)
			d.Nack(false, true)
			return
		}
	...
	logger.Printf(ctx, "[Users Consumer] Internal error for Workflow %s: %v", cmd.WorkflowID, err)
	d.Nack(false, true)
}
